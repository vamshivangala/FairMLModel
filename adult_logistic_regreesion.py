...
X = data.drop(['income', 'age_group'], axis=1)  # Ensure 'age_group' is not in X
y = data['income']
A = data[protected_attributes]
...
evaluate_model(y_test_bl, y_pred_baseline, "Baseline Logistic Regression")

# Calculate fairness metrics
calculate_fairness_metrics(y_test_bl, y_pred_baseline, A_test_bl, "Baseline Logistic Regression")

# ----------------------------------------
# Blinded Model (With Blinding)
# ----------------------------------------

print("\n********** Blinded Logistic Regression Model **********")

# Remove protected attributes from features
X_blinded = X.drop(protected_attributes, axis=1)
A_blinded = A.copy()
...
lr_blinded = LogisticRegression(max_iter=1000)
lr_blinded.fit(X_train_bd, y_train_bd)
y_pred_blinded = lr_blinded.predict(X_test_bd)

# Evaluate the blinded model
evaluate_model(y_test_bd, y_pred_blinded, "Blinded Logistic Regression")

# Calculate fairness metrics
calculate_fairness_metrics(y_test_bd, y_pred_blinded, A_test_bd, "Blinded Logistic Regression")

# ----------------------------------------
# Fairness-Adjusted Model (In-processing)
# ----------------------------------------

print("\n********** Fairness-Adjusted Logistic Regression Model **********")

# Use only 'race' as protected attributes
...
eg_lr = ExponentiatedGradient(estimator=estimator, constraints=constraint)

# Fit the model with fairness constraint
eg_lr.fit(X_train_bl, y_train_bl, sensitive_features=A_train_bl)

# Make predictions
y_pred_eg = eg_lr.predict(X_test_bl)

# Evaluate the fairness-adjusted model
evaluate_model(y_test_bl, y_pred_eg, "Fairness-Adjusted Logistic Regression")

# Calculate fairness metrics
calculate_fairness_metrics(y_test_bl, y_pred_eg, A_test_bl, "Fairness-Adjusted Logistic Regression")
